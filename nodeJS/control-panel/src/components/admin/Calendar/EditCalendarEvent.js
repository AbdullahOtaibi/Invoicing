...
import { updateCalendarEvent, getCalendarEvent } from './CalendarAPI'
...
const EditCalendarEvent = (props) => {
    ...
    useEffect(() => {
        getCalendarEvent(eventId).then(res => {
            setArticle(res.data);
           // alert(res.data.eventDate);
            setSelectedDate(Date.parse(res.data.eventDate));
        });
    }, []);
    ...
    const doPost = data => {
        setLoading(true);
        updateCalendarEvent(article).then(res => {
            setLoading(false);
