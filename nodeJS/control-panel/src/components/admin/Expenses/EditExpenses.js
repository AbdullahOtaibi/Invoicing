...
import { MdClose, MdCollections, MdContacts, MdTurnedInNot } from "react-icons/md";
import "react-datepicker/dist/react-datepicker.css";
import { CSSTransition } from 'react-transition-group';
import { getPackage, updatePackage } from "./ExpensesAPI"
...
const EditExpenses = (props) => {
  ...
  const doPost = (event) => {
    ...
    if (checkData()) {
      updatePackage(Package).then((res) => {
        toast("success!");
        window.location.href = "/admin/Package/view/" + res._id;
    ...
    <>
      <div className="card">
        <div className="card-body">
          <h5 className="card-title"><MdCollections size={20} />  {t("Package.editPackage")}</h5>
