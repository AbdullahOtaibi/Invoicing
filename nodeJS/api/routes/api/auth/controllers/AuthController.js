...
router.post('/login', async (req, res) => {
        const user = await User.findOne({ email: req.body.email }).populate("roles").populate({
                path: 'roles',
                populate: {
                        path: 'permissions',
                        model: 'Permission'
...
router.get('/oauth-login/:email', async (req, res) => {
        const user = await User.findOne({ email: req.params.email }).populate("roles").populate({
                path: 'roles',
                populate: {
                        path: 'permissions',
                        model: 'Permission'
...
router.get('/oauth-profile/:token', async (req, res) => {
        ...
        if (typeof req.params.token !== 'undefined') {
                ...
                await jwt.verify(bearerToken, process.env.ACCESS_TOKEN_SECRET, (err, authData) => {
                        if (err) {
                                res.sendStatus(403);
                        } else {
        ...
        user = await User.findOne({ email: user.email }).populate("roles").populate({
                path: 'roles',
                populate: {
                        path: 'permissions',
                        model: 'Permission'
...
router.post('/register', async (req, res) => {
        ...
        newUser.save().then(user => {
                ...
                let mailBody = "Welcome " + user.firstName + " " + user.surName + "<br/> Please click <a href='https://waredly.com/activate-account/" + user._id + "/" + rand + "'> here </a> to activate your account.";
